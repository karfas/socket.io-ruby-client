...
class WebSocket
	...
	def initialize(arg, params = {})
		if params[:server] # server
			...
			unless @server.accepted_origin?(self.origin)
	...
	def tcp_socket; @socket end

	def host; @header['host'] end
	...
	def location; "ws://#{self.host}#{@path}" end
	...
	def close_socket; @socket.close() end
	...
	def read_header
		...
		unless @header['connection'] =~ /\AUpgrade\z/i
	...
	def security_digest(key)
		Base64.encode64(Digest::SHA1.digest(key + WEB_SOCKET_GUID)).gsub(/\n/, '')
	end
